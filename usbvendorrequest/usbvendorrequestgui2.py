...
class usbvendorrequestgui2(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        if self.dev.dev >= 0:
            ...
            self.root = tk.Tk()
            self.root.title('Logans usb vendor request GUI version 1 test')
    ...
    def buttons(self):
        ...
        soup = tk.Button(self.master, text = 'Test1 Button', command = self.dev.toggle_led1, bg=a, fg=d, activebackground=b, font= fo)
        ...
        ledone = tk.Button(tk.Frame(self.root), text = 'LED ONE', command = self.dev.toggle_led1)
        ledone.grid(row=1,column=3, padx=20)
    ...
    def words(self):
        title = tk.Label(self.master, text = "title line 44 here ", bg=c,fg=d, font= (fo,18))
...
infotext = """ infotext is here.
    This can be a description of what the different modes do, or about the goals of the project  """

alltext ="""This section is in development. Please be careful. """
